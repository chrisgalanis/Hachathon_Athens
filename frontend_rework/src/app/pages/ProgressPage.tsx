...
import { Flame, Target, Award, Sparkles, Calendar } from 'lucide-react';
...
const achievements = [
  ...
  { id: 5, icon: '💎', title: '1000 Aura',      desc: 'Earn 1000 total Aura',              unlocked: false, color: '#ec4899' },
  { id: 6, icon: '🌟', title: 'Perfect Week',   desc: 'Complete daily goal 7 days',        unlocked: false, color: '#8b5cf6' },
];

const weeklyActivity = [
  { day: 'Mon', aura: 120, completed: true  },
  { day: 'Tue', aura: 85,  completed: true  },
  { day: 'Wed', aura: 150, completed: true  },
  { day: 'Thu', aura: 95,  completed: true  },
  { day: 'Fri', aura: 110, completed: true  },
  { day: 'Sat', aura: 0,   completed: false },
  { day: 'Sun', aura: 0,   completed: false },
];

export function ProgressPage() {
  const totalAura        = 2847;
        ...
        >
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-white/60 text-sm">Total Aura</span>
            </div>
            <div className="text-5xl text-white mb-2">{totalAura.toLocaleString()}</div>
            <div className="text-[#06b6d4] text-sm">+560 this week</div>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="flex items-end justify-between gap-2 h-32 mb-4">
            {weeklyActivity.map((day, i) => {
              const maxAura = Math.max(...weeklyActivity.map(d => d.aura));
              const height  = (day.aura / maxAura) * 100;
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-2">
                  <motion.div
                    className="w-full rounded-lg relative overflow-hidden"
                    style={{
                      height: day.aura > 0 ? `${height}%` : '8px',
                      background: day.completed
                        ? 'linear-gradient(to top, #7c3aed, #06b6d4)'
                        : 'rgba(255,255,255,0.1)',
                    }}
                    initial={{ height: 0 }}
                    animate={{ height: day.aura > 0 ? `${height}%` : '8px' }}
          ...
          <div className="grid grid-cols-3 gap-3">
            {achievements.map((achievement, i) => (
              <motion.div
                ...
                className={`relative backdrop-blur-xl border rounded-2xl p-4 text-center transition-all ${
              ...
              >
                ...
                <div className="text-white text-xs mb-1 line-clamp-1">{achievement.title}</div>
                <div className="text-white/40 text-[10px] line-clamp-2">{achievement.desc}</div>
                {achievement.unlocked && (
                  <motion.div
